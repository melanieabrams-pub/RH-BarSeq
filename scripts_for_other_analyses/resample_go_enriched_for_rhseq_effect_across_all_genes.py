...
#usage: resample_go_enriched_in_goi.py # (modify parameters below, first, to file paths and GOI and cutoffs)
#use: resample barcoded RH-seq fitness data for ontology terms enriched for low fitness when the S. cerevisae allele is knocked out.
#this script can take a long time to run for a large dataset; in that case, we recommend first running with a lower n sample size, then with a higher n feeding the script the lower n file so it does not compute to a higher confidence values that are very far from significance.
...
low_n_file='resample_all_go_by_absolute_fxsize_n100_onlyBioProcess-False.tsv' #if have already run the code with low n (n=100, then 1000), to speed up computation of high n p-values for candidates with rv<0.1
...
def resampleAllGo(go_terms,fx_df,essential_go_counts,non_essential_go_counts,low_n_file=low_n_file,n=10000):
    ...
    rv_df.to_csv('resample_all_go_by_absolute_fxsize_n'+str(n)+'_onlyBioProcess-'+str(onlyBioProcess)+'.tsv', sep='\t', index=True)
...
if __name__ == "__main__":
    ...
    #all_go_counts.to_csv('all_go_counts.tsv',sep=sep) #uncomment for additional outfile
