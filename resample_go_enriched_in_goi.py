...
#use: resample GOI for ontology terms enriched among those genes
...
def resampleAllGo(go_term_groups,goi,go_terms,essential_count,non_essential_count,n=10000,save_intermediate=True):
    ...
    rv_df.to_csv('resample_go_enriched_in_goi_'+str(min_go_count)+'max'+str(max_go_count)+'_mingoi'+str(min_goi_count)+'onlyBioprocess'+str(onlyBioprocess)+'.tsv', sep='\t', index=True)
...
if __name__ == "__main__":
    ...
    #all_go_counts.to_csv('all_go_counts.tsv',sep=sep) #can uncomment to generate this additional outfile
